...
CREATE OR REPLACE FUNCTION private.can_manage_flyer_thumbnail(_object_name text)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
GRANT EXECUTE ON FUNCTION private.can_manage_flyer_thumbnail(text) TO authenticated;
...
CREATE POLICY "Owner can insert flyer thumbnail"
  ...
  WITH CHECK (
    ...
    AND private.can_manage_flyer_thumbnail(name)
  );
...
DROP FUNCTION IF EXISTS public.can_manage_flyer_thumbnail(text);